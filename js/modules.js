...
function  menu() {
    ...
    if(mediaQuery1024){
        ...
        document.querySelector('.header_menu > div').addEventListener('click', () => {
            countCLick++
            if(countCLick % 2){
                ...
                menu.style.left = (menuWidth * -1) - 50 + 'px'
                document.querySelector('.header_menu-icon img').src = 'img/header/menu.svg'
            }         
    ...
    let trigerForAnimationNext = (trigerForAnimation + 100) * -1
        ...
        menuItems.forEach((item, index) => {
            scrollToMyPersonalSection(`.menu_list li:nth-child(${index+1})`, item.dataset.menuanchor)
        })
...
function  footer() {
    ...
    document.querySelectorAll('.footer_menu li').forEach((item, index) => {
         scrollToMyPersonalSection(`.footer_menu li:nth-child(${index+1})`, item.dataset.menuanchor)
    })
}